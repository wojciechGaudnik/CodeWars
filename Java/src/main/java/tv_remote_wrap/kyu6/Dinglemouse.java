...
public class Dinglemouse {
	...
	private static void calculate(int[] pass, char one) {
		pass[answer] += Math.min(Math.abs(remote.get(one)[0] - pass[y]), Math.min(remote.get(one)[0], pass[y]) + 6 - Math.max(remote.get(one)[0], pass[y]));
		pass[answer] += Math.min(Math.abs(remote.get(one)[1] - pass[x]), Math.min(remote.get(one)[1], pass[x]) + 8 - Math.max(remote.get(one)[1], pass[x]));
		pass[answer]++;
